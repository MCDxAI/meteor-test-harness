...
import io.mcdxai.harness.universal.dom.DomInteractor;
import io.mcdxai.harness.universal.dom.DomQueryEngine;
import io.mcdxai.harness.universal.dom.DomSnapshot;
import io.mcdxai.harness.universal.dom.DomSnapshotBuilder;
...
public final class ScreenDomService {
    ...
    public ScreenDomService(AdapterRegistry registry) {
        this.snapshotBuilder = new DomSnapshotBuilder(registry);
        this.queryEngine = new DomQueryEngine();
        this.interactor = new DomInteractor(snapshotBuilder, queryEngine, registry);
    }
    ...
    public synchronized Map<String, Object> findElements(String snapshotId, Map<String, Object> filters, int limit, List<Object> fieldsRaw, boolean includeChildren) {
    ...
    public synchronized Map<String, Object> getElement(String snapshotId, String elementId, List<Object> fieldsRaw, boolean includeChildren) {
        DomSnapshot snapshot = snapshotBuilder.resolveSnapshotForRead(snapshotId, true);
        return queryEngine.getElement(snapshotId, snapshot, elementId, fieldsRaw, includeChildren);
    }

    public synchronized Map<String, Object> getSubtree(String snapshotId, String elementId, int depth, List<Object> fieldsRaw) {
        DomSnapshot snapshot = snapshotBuilder.resolveSnapshotForRead(snapshotId, true);
        return queryEngine.getSubtree(snapshotId, snapshot, elementId, depth, fieldsRaw);
    }

    public synchronized Map<String, Object> clickByQueryDetailed(Map<String, Object> filters, int index, int button, boolean doubled) {
        return interactor.clickByQueryDetailed(filters, index, button, doubled);
    }

    public synchronized Map<String, Object> setTextByQueryDetailed(Map<String, Object> filters, int index, String text, boolean submit, boolean typeCharacters, boolean clearFirst) {
