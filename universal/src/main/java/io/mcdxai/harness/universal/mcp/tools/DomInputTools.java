...
public final class DomInputTools {
    ...
    public static void register(List<McpServerFeatures.SyncToolSpecification> tools, RegistryContext context) {
        ...
        // Input tools run on the MCP thread (runOnMainThread=false) so the DomInteractor's
        // internal settle sleep doesn't block the render thread.
        ...
        tools.add(context.tool(
            ...
            (exchange, args) -> {
                ...
                if (!helper.isSuccess(interaction)) {
                    return McpResults.error("Element does not accept text input or filter rejected value.", helper.errorDetails(interaction));
                }
